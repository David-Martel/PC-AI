...
fn test_checkpoint_workflow() {
    ...
    checkpoint.save(&ckpt_path).expect("checkpoint save should succeed");

    // Verify it can be loaded
    let loaded = Checkpoint::load(&ckpt_path).expect("checkpoint load should succeed");
...
fn test_checkpoint_cleanup() {
    ...
    Checkpoint::cleanup_old(&checkpoint_config).expect("checkpoint cleanup should succeed");
