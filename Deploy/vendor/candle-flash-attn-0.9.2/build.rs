...
fn detect_cuda_major_version() -> u32 {
    ...
    for line in stdout.lines() {
        if let Some(pos) = line.find("release ") {
            ...
            if let Some(dot) = version_str.find('.') {
                return version_str[..dot].parse::<u32>().unwrap_or(0);
            }
        }
    }
    0
...
fn main() -> Result<()> {
    ...
    if let Ok(target) = std::env::var("TARGET") {
        if target.contains("msvc") {
            ...
            // CUDA 13+ CCCL headers require the standard conforming preprocessor.
            if detect_cuda_major_version() >= 13 {
                builder = builder
