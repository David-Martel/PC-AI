...
.PARAMETER DollarNullOnly
    Delete only real files whose visible leaf is literal $null.
...
if (-not $UsePowerShell -and $null -ne $NukeNulExe) {
    ...
    try {
        ...
        $NukeNulArgs = @()
        if ($DollarNullOnly) {
            $NukeNulArgs += '--dollar-null-only'
        }
        $NukeNulArgs += $SearchPath
        $Output = & $NukeNulExe @NukeNulArgs 2>&1 | Out-String
...
$ReservedNames = if ($DollarNullOnly) {
    @([char]36 + 'null')
}
else {
    @("nul", "con", "prn", "aux") + (1..9 | ForEach-Object { "com$_", "lpt$_" })
}
...
function Get-ReservedFiles {
    ...
    while ($stack.Count -gt 0) {
        ...
        try {
            ...
            if ($DollarNullOnly) {
                [System.IO.Directory]::EnumerateFiles($currentDir) |
                    Where-Object {
                        [System.IO.Path]::GetFileName($_).TrimEnd([char[]]@('.', ' ')) -ieq ([char]36 + 'null')
                    }
            }
            else {
