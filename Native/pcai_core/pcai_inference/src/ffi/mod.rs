...
/// * Must call pcai_load_model first
#[no_mangle]
#[expect(unused_variables, reason = "FFI boundary parameters required by the C ABI contract; some are unused when the llamacpp feature is disabled")]
...
pub extern "C" fn pcai_generate_async(prompt: *const c_char, max_tokens: u32, temperature: f32) -> i64 {
    ...
    std::thread::spawn(move || {
        ...
        if let Ok(mut g) = init_global_state().lock() {
            ...
            match result {
                Ok(response) => {
                    g.requests.insert(id, RequestStatus::Complete(response.text));
                }
                Err(e) => {
