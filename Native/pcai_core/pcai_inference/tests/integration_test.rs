...
fn test_generate_request_serialization() {
    ...
    let deserialized: GenerateRequest = serde_json::from_str(&json).expect("deserialization should succeed");
...
fn test_generate_response_serialization() {
    ...
    let deserialized: GenerateResponse = serde_json::from_str(&json).expect("deserialization should succeed");
...
fn test_finish_reason_serialization() {
    ...
    for (reason, expected) in reasons {
        ...
        let deserialized: FinishReason = serde_json::from_str(&json).expect("deserialization should succeed");
...
fn test_backend_type_llamacpp() {
    ...
    let backend = backend_result.expect("backend should initialize successfully");
    assert_eq!(backend.backend_name(), "llama.cpp");
    assert!(!backend.is_loaded());
}
...
async fn test_mock_backend_lifecycle() {
    ...
    // Unload model
    backend.unload_model().await.expect("async operation should succeed");
    assert!(!backend.is_loaded());
}
...
async fn test_mock_backend_generate() {
    ...
    let response = backend.generate(request).await.expect("async operation should succeed");
...
mod ffi_tests {
    ...
    fn test_ffi_full_lifecycle_with_model() {
        ...
        let text = result_str.to_str().expect("result should be valid UTF-8");
...
async fn stress_test_backend_switching() {
    ...
    // Test llamacpp
    {
        ...
        let _ = backend.generate(request).await.expect("async operation should succeed");
        backend.unload_model().await.expect("async operation should succeed");
    }
