...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=GGML_CUDA_ARCH_LIST");
    println!("cargo:rerun-if-env-changed=PCAI_CUDA_ARCH_LIST");
...
fn parse_cuda_arch_token(token: &str) -> Option<u32> {
    ...
    let arch = if let Some((major, minor)) = normalized.split_once('.') {
...
fn parse_cuda_arch_list(raw: &str) -> Vec<u32> {
    let mut arches: Vec<u32> = raw
        .split(|c: char| c == ',' || c == ';' || c.is_whitespace())
        .filter_map(parse_cuda_arch_token)
        .collect();
...
fn detect_cuda_arches_with_nvidia_smi() -> Vec<u32> {
    ...
    let mut arches: Vec<u32> = stdout.lines().filter_map(parse_cuda_arch_token).collect();
    arches.sort_unstable();
    arches.dedup();
    arches
}

fn default_cuda_arches() -> Vec<u32> {
    // Conservative multi-GPU fallback that avoids legacy arch values removed in modern CUDA toolkits.
    vec![61, 70, 75, 80, 86]
}

fn resolve_cuda_arches() -> Vec<u32> {
    if let Ok(raw) = env::var("GGML_CUDA_ARCH_LIST") {
        let arches = parse_cuda_arch_list(&raw);
        if !arches.is_empty() {
            println!("cargo:warning=Using GGML_CUDA_ARCH_LIST={raw}");
            return arches;
        }
    }

    if let Ok(raw) = env::var("PCAI_CUDA_ARCH_LIST") {
        let arches = parse_cuda_arch_list(&raw);
        if !arches.is_empty() {
            println!("cargo:warning=Using PCAI_CUDA_ARCH_LIST={raw}");
            return arches;
        }
    }

    let detected = detect_cuda_arches_with_nvidia_smi();
    if !detected.is_empty() {
        let list = detected
            ...
            .collect::<Vec<_>>()
            .join(",");
        println!("cargo:warning=Auto-detected CUDA arch list via nvidia-smi: {list}");
        return detected;
    }

    let fallback = default_cuda_arches();
    let list = fallback
        .iter()
        .map(u32::to_string)
        .collect::<Vec<_>>()
        .join(",");
    println!("cargo:warning=Using default CUDA arch list: {list}");
    fallback
}
...
fn enable_cublas(build: &mut cc::Build, out_dir: &Path) {
    ...
    let cuda_arches = resolve_cuda_arches();
    if cuda_arches.is_empty() {
        panic!("No valid CUDA architectures resolved. Set GGML_CUDA_ARCH_LIST or PCAI_CUDA_ARCH_LIST.");
    }
    ...
    if cfg!(windows) {
        ...
        if !status.success() {
            panic!(
                "nvcc failed with status {status}. Set GGML_CUDA_ARCH_LIST/PCAI_CUDA_ARCH_LIST to supported values."
            );
        }
